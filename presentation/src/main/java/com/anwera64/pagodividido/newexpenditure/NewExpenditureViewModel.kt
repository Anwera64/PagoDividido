...
class NewExpenditureViewModel(private val useCase: NewExpenditureUseCase) : ViewModel() {
    ...
    fun createExpenditure(
        ...
        val payerIdError = useCase.checkPayerInputError(payerId)
        _payerInputError.postValue(payerIdError)
        val amountInputError = useCase.checkAmountInputError(amountString)
        ...
        val errors: List<DebtorInputError> = useCase.lookForDebtorInputErrors(amountSpent, debtors)
        if (errors.isNotEmpty()) {
            _debtInputErrors.postValue(errors)
            return
        }

        val amountsDifference = useCase.checkAmountsDifference(amountSpent, debtors)
    ...
    fun getCompanions(tripId: Int) {
        _companions.addSource(
            useCase.getTripCompanions(tripId).asLiveData(),
